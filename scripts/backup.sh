...
# Cron via host crontab: 30 3 * * * /home/lakshit_gupta/coding/cartograph/scripts/backup.sh
...
DUMP_FILE="$TMP_DIR/cartograph_$DATE.sql.gz"
...
AGE_RECIPIENTS_FILE="${AGE_RECIPIENTS_FILE:-/etc/cartograph/age_recipients.txt}"
...
sops exec-env secrets.yaml '
  rclone --config /etc/cartograph/rclone.conf copy "'"$ENC_FILE"'" r2:'$r2_bucket'/postgres/
'
