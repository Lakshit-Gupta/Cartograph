...
def _ei(payload: dict | list, *, source_id: int = 1, slug: str = "x",
        url: str = "https://example.com") -> ExtractInput:
    return ExtractInput(
        source_id=source_id, source_slug=slug, url=url,
...
async def test_lever_basic_extraction():
    out = await lever_extract(_ei([{
        ...
        "categories": {"team": "Platform", "department": "Engineering",
                       "location": "Remote - US", "commitment": "Full-time"},
    ...
    out = await ashby_extract(_ei({
        "jobs": [{
            ...
            "compensation": {"compensationTierSummary": {
                "minValue": 200000, "maxValue": 250000, "currencyCode": "USD",
            }},
        }],
    }))
    assert len(out.opps) == 1
...
async def test_workable_basic_extraction():
    out = await workable_extract(_ei({
        "jobs": [{
...
async def test_workable_intern_classification():
    out = await workable_extract(_ei({"jobs": [{
...
async def test_contra_handles_null_budget():
    ...
    out = await contra_extract(_ei({"opportunities": [{
