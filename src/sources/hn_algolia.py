...
class _HNAlgolia:
    ...
    async def plan(self, *, source_id: int, base_url: str, config: dict) -> CrawlPlan:
        ...
        story_search = f"{base_url}?tags=story&query=Ask%20HN%20Who%20is%20hiring&hitsPerPage=1"
        comments_search = f"{base_url}?tags=comment,story_3148259&hitsPerPage=400"  # placeholder story id
        return CrawlPlan(
            source_id=source_id,
