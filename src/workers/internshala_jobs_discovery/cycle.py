...
_OP_TIMEOUT_MS = 15_000  # default cap for every other page op (content/click/selector)
...
async def run_combo(
    ...
    async with engine.open_context(cookies=cookies, ua=ua) as ctx:
        ...
        # Hard-cap EVERY page op. Internshala's logged-in page + persistent
        # telemetry can stall `domcontentloaded` indefinitely; without an explicit
        # navigation timeout `goto` hangs forever and the asyncio wall-clock can't
        # cancel a stuck CDP await. These bound goto + every selector/content/click.
        page.set_default_navigation_timeout(_NAV_TIMEOUT_MS)
        page.set_default_timeout(_OP_TIMEOUT_MS)
        try:
            await page.goto(variant.url, wait_until="domcontentloaded", timeout=_NAV_TIMEOUT_MS)
